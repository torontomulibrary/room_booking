	...
	**/
	...
	function roles(){
		...
		if($this->uri->segment(3) === 'add'){
			...
			$id = $this->role_model->add_role($role_name, $bookings_day, $hours_day, $hours_week, $booking_window);
		...
		//Set variable so the view loads the form, rather then list out existing roles
			...
			$id = $this->role_model->edit_role($role_id, $role_name, $bookings_day, $hours_day, $hours_week, $booking_window);
